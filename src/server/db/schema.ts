...
// Game tables
export const game = sqliteTable("game", (d) => ({
	id: d.integer({ mode: "number" }).primaryKey({ autoIncrement: true }),
	name: d.text({ length: 256 }).notNull(),
	url: d.text({ length: 512 }).notNull().unique(),
...
export const gameProgress = sqliteTable(
	...
	(t) => [index("user_game_date_idx").on(t.userId, t.gameId, t.playedAt)],
);
...
export const gameProgressRelations = relations(gameProgress, ({ one }) => ({
	game: one(game, { fields: [gameProgress.gameId], references: [game.id] }),
	user: one(user, { fields: [gameProgress.userId], references: [user.id] }),
}));
